...
#for i in range(len(spline)):
    #canvas = cv2.line(canvas, spline[i], spline[i-1], (128, 128, 0), 1, lineType=cv2.LINE_AA)
    #canvas = cv2.putText(canvas, str(fraction[i]), (spline[i][0]+5, spline[i][1]+5), fontFace=cv2.FONT_HERSHEY_SIMPLEX, fontScale=.3, color=(128, 128, 0) )
#for i in range(len(spline)):
